...
exports.find = function(req, res, next) {
    console.log('find function of /admin/users/');
    console.log(req.query);
    req.query.username = req.query.username ? req.query.username : '';
    req.query.limit = req.query.limit ? parseInt(req.query.limit, null) :
        20;
    ...
    if (req.query.isActive) {
        console.log('isActive if');
        filters.isActive = req.query.isActive;
        console.log('filters.isActive is:' + filters.isActive);
    }
    console.log('333');
    if (req.query.roles && req.query.roles === 'admin') {
        ...
        console.log('filters.username is:' + filters['roles.admin']);
    }
...
exports.read = function(req, res, next) {
    console.log('in read function of admin/users');
    req.app.db.models.User.findById(req.params.id).populate('roles.admin',
        'name.full').populate('roles.account', 'name.full').exec(
        function(err, user) {
            if (err) {
                return next(err);
            }
...
exports.create = function(req, res, next) {
    console.log('in create function of admin/users');
    ...
    workflow.on('createUser', function() {
        ...
        req.app.db.models.User.create(fieldsToSet, function(err,
            user) {
            if (err) {
                return workflow.emit('exception', err);
            }
...
exports.update = function(req, res, next) {
    console.log('in update function of admin/users');
    var workflow = req.app.utility.workflow(req, res);
    ...
    workflow.on('patchAdmin', function(user) {
        if (user.roles.admin) {
            ...
            req.app.db.models.Admin.findByIdAndUpdate(user.roles.admin,
                fieldsToSet,
                function(err, admin) {
                    if (err) {
                        return workflow.emit('exception', err);
    ...
    workflow.on('patchAccount', function(user) {
        if (user.roles.account) {
            ...
            req.app.db.models.Account.findByIdAndUpdate(user.roles.account,
                fieldsToSet,
    ...
    workflow.on('populateRoles', function(user) {
        user.populate('roles.admin roles.account', 'name.full',
...
exports.password = function(req, res, next) {
    console.log('in password function of admin/users');
    var workflow = req.app.utility.workflow(req, res);
    ...
    workflow.on('patchUser', function() {
        req.app.db.models.User.encryptPassword(req.body.newPassword,
            function(err, hash) {
                if (err) {
                    return workflow.emit('exception', err);
                }
                ...
                req.app.db.models.User.findByIdAndUpdate(req.params
                    .id, fieldsToSet,
                    function(err, user) {
                        if (err) {
...
exports.linkAdmin = function(req, res, next) {
    console.log('in linkAdmin function of admin/users');
    var workflow = req.app.utility.workflow(req, res);

    workflow.on('validate', function() {
        if (!req.user.roles.admin.isMemberOf('root')) {
            workflow.outcome.errors.push(
                'You may not link users to admins.');
            return workflow.emit('response');
        }
    ...
    workflow.on('verifyAdmin', function(callback) {
        req.app.db.models.Admin.findById(req.body.newAdminId).exec(
            function(err, admin) {
                if (err) {
                    return workflow.emit('exception', err);
                }
                ...
                if (admin.user.id && admin.user.id !== req.params
                    .id) {
                    workflow.outcome.errors.push(
                        'Admin is already linked to a different user.'
                    );
                    return workflow.emit('response');
                }
    ...
    workflow.on('duplicateLinkCheck', function(callback) {
        req.app.db.models.User.findOne({
            ...
            if (user) {
                workflow.outcome.errors.push(
                    'Another user is already linked to that admin.'
                );
                return workflow.emit('response');
            }
    ...
    workflow.on('patchUser', function(callback) {
        req.app.db.models.User.findById(req.params.id).exec(
            function(err, user) {
                if (err) {
                    return workflow.emit('exception', err);
                }
...
exports.unlinkAdmin = function(req, res, next) {
    console.log('in unlinkadmin function of admin/users');
    var workflow = req.app.utility.workflow(req, res);

    workflow.on('validate', function() {
        if (!req.user.roles.admin.isMemberOf('root')) {
            workflow.outcome.errors.push(
                'You may not unlink users from admins.');
            return workflow.emit('response');
        }

        if (req.user._id === req.params.id) {
            workflow.outcome.errors.push(
                'You may not unlink yourself from admin.');
            return workflow.emit('response');
        }
    ...
    workflow.on('patchAdmin', function(id) {
        req.app.db.models.Admin.findById(id).exec(function(err,
            admin) {
            if (err) {
                return workflow.emit('exception', err);
            }
...
exports.linkAccount = function(req, res, next) {
    console.log('in linkAccount function of admin/users');
    var workflow = req.app.utility.workflow(req, res);

    workflow.on('validate', function() {
        if (!req.user.roles.admin.isMemberOf('root')) {
            workflow.outcome.errors.push(
                'You may not link users to accounts.');
            return workflow.emit('response');
        }
    ...
    workflow.on('verifyAccount', function(callback) {
        req.app.db.models.Account.findById(req.body.newAccountId).exec(
            function(err, account) {
                if (err) {
                    return workflow.emit('exception', err);
                }
                ...
                if (account.user.id && account.user.id !== req.params
                    .id) {
                    workflow.outcome.errors.push(
    ...
    workflow.on('duplicateLinkCheck', function(callback) {
        req.app.db.models.User.findOne({
            ...
            if (user) {
                workflow.outcome.errors.push(
                    'Another user is already linked to that account.'
                );
                return workflow.emit('response');
            }
...
exports.unlinkAccount = function(req, res, next) {
    ...
    workflow.on('validate', function() {
        if (!req.user.roles.admin.isMemberOf('root')) {
            workflow.outcome.errors.push(
                'You may not unlink users from accounts.');
            return workflow.emit('response');
        }
    ...
    workflow.on('patchAccount', function(id) {
        req.app.db.models.Account.findById(id).exec(function(err,
            account) {
            if (err) {
                return workflow.emit('exception', err);
            }
...
exports.delete = function(req, res, next) {
    ...
    workflow.on('validate', function() {
        if (!req.user.roles.admin.isMemberOf('root')) {
            workflow.outcome.errors.push(
                'You may not delete users.');
            return workflow.emit('response');
        }

        if (req.user._id === req.params.id) {
            workflow.outcome.errors.push(
                'You may not delete yourself from user.');
            return workflow.emit('response');
        }
