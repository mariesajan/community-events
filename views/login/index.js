...
var getReturnUrl = function(req) {
    console.log('in getReturnUrl function');
...
exports.login = function(req, res) {
    ...
    workflow.on('abuseFilter', function() {
        var getIpCount = function(done) {
            ...
            console.log('in login/reset/index.js file...');
            req.app.db.models.LoginAttempt.count(conditions,
                function(err, count) {
    ...
    workflow.on('attemptLogin', function() {
        req._passport.instance.authenticate('local', function(err,
            user, info) {
            if (err) {
                return workflow.emit('exception', err);
            }
            ...
            if (!user) {
                ...
                req.app.db.models.LoginAttempt.create(
                    fieldsToSet,
                    function(err, doc) {
                        if (err) {
        ...
        req.app.db.models.User.findOne({
            ...
            if (!user) {
                res.render('login/index', {
                    ...
                    oauthTwitter: !!req.app.config.oauth
                        .twitter.key,
                    oauthGitHub: !!req.app.config.oauth
                        .github.key,
                    oauthFacebook: !!req.app.config
                        .oauth.facebook.key,
                    oauthGoogle: !!req.app.config.oauth
                        .google.key,
                    oauthTumblr: !!req.app.config.oauth
                        .tumblr.key
                });
            } else {
                req.login(user, function(err) {
